...
function FavoriteList({ favorites, onRemoveFavorite, loading, onRefesh }) {
    return (
        <div>
            <div className = "favorites-header">
                <h2 className = "section-title">내가 찜한 목록</h2>
                <button onclick = {onRefesh} disabled = {loading} className = "btn btn-primary">
                    {loading ? '로딩중 ...' : '새로고침'}
                </button>
            </div>
                ...
                <div className = "favorites-grid">
                    {favorites.map(favorite => (
                        <div key = {favorite.id} className = "favorite-card">
                            ...
                            <button onClick = {() => onRemoveFavorite(favorite.id, favorite.title)} 
